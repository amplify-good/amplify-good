...
import { musicians, allGenres, type Musician } from "@/data/musicians";
...
export default function MusiciansPage() {
  ...
  const filtered = useMemo(() => {
    return musicians.filter((m: Musician) => {
      const matchesName =
        nameSearch.trim() === "" ||
      ...
      const matchesMin = min === null || m.rate >= min;
      const matchesMax = max === null || m.rate <= max;

      return matchesName && matchesGenres && matchesMin && matchesMax;
    });
  }, [nameSearch, selectedGenres, minRate, maxRate]);
